...
Server::Server()
    ...
    , plaidItemRepo_(db_)
    , pfcPrimaryRepo_(db_)
    , pfcDetailedRepo_(db_)
    , openai_(config_.openAiApiKey(), config_.openAiModel())
    , promptBuilder_()
    , plaid_(config_.plaidClientId(), config_.plaidSecret(), config_.plaidEnv())
    ...
    , getUserSettingsView_(userRepo_, countryRepo_, currencyRepo_, languageRepo_, employmentStatusRepo_)
    , createChatSession_(chatSessionRepo_)
    , deleteChatSession_(chatSessionRepo_)
    , getChatHistory_(chatMessageRepo_)
    , getChatSessionView_(chatSessionRepo_, personaRepo_)
    , listChatSessionsView_(chatSessionRepo_, personaRepo_)
    , sendChatMessage_(chatSessionRepo_, chatMessageRepo_, personaRepo_, transactionRepo_, pfcDetailedRepo_, openai_, promptBuilder_, getUserProfile_)
    ...
    , importPlaidTransactions_(transactionRepo_, currencyRepo_, pfcDetailedRepo_, plaid_, plaidItemRepo_)
    , createPlaidLinkToken_(plaid_)
    , linkPlaidAccount_(plaid_, plaidItemRepo_, importPlaidTransactions_)
    ...
    , getFinancialInsights_(transactionRepo_, pfcDetailedRepo_)
    , getPfcPrimaryCategory_(pfcPrimaryRepo_)
    , listPfcPrimaryCategories_(pfcPrimaryRepo_)
    , getPfcDetailedCategory_(pfcDetailedRepo_)
    , listPfcDetailedCategories_(pfcDetailedRepo_)
    , homeController_()
    , authController_(loginUser_, registerUser_, loginOrRegisterOAuthUser_, googleOAuth_, jwt_)
    , userController_(getUserProfile_, updateUserProfile_, updateUserPassword_, deleteUser_, getFinancialInsights_, getUserSettingsView_)
    , userChatController_(createChatSession_, updateChatSession_, deleteChatSession_, getChatSessionView_, listChatSessionsView_, getChatHistory_, sendChatMessage_, getUserProfile_)
    , userTransactionsController_(createTransactions_, deleteTransactions_, getTransactions_, getUserProfile_, listTransactions_, listTransactionsByCategory_, updateTransactions_, importPlaidTransactions_, createPlaidLinkToken_, linkPlaidAccount_)
    , referenceController_(getCountry_, listCountries_, getCurrency_, listCurrencies_, getEmploymentStatus_, listEmploymentStatuses_, getLanguage_, listLanguages_, getLLMPersona_, listLLMPersonas_, getMessageSender_, listMessageSenders_, getTransactionCategory_, listTransactionCategories_, getPfcPrimaryCategory_, listPfcPrimaryCategories_, getPfcDetailedCategory_, listPfcDetailedCategories_)
{}
