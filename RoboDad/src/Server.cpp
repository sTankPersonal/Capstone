...
Server::Server()
    ...
    , emailService_(config_.sendGridApiKey(), config_.emailFromAddress(), config_.appBaseUrl())
    , jwt_(config_.jwtSecret())
    , hasher_()
    , loginUser_(userRepo_, hasher_)
    , registerUser_(userRepo_, hasher_, emailService_)
    , loginOrRegisterOAuthUser_(userRepo_, hasher_)
    , verifyEmail_(userRepo_)
    , resendVerificationEmail_(userRepo_, emailService_)
    ...
    , homeController_()
    , authController_(loginUser_, registerUser_, loginOrRegisterOAuthUser_, verifyEmail_, resendVerificationEmail_, googleOAuth_, jwt_)
    , userController_(getUserProfile_, updateUserProfile_, updateUserPassword_, deleteUser_, getFinancialInsights_)
